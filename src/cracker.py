...
def refine_from_key(ciphertext, key, trigrams, quadgrams, iterations=3000):
    """Affine une clé de départ (ex: partielle depuis le solveur par motifs)
    via un recuit simulé, au lieu de repartir d'une clé aléatoire."""

    current_key = key
    current_text = decrypt(ciphertext, current_key)
    current_score = score_text(current_text, trigrams, quadgrams)
